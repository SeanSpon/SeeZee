...
export default function AssignResourceModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/ceo/resource-assignments", {
        method: "POST",
      ...
      // Show success toast
      toast(
        `Successfully assigned resource to ${audience.userIds.length} user(s)`,
        "success"
      );
      ...
      toast(err.message || "Failed to assign resource", "error");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="glass w-full max-w-2xl rounded-lg border border-slate-700 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 border-b border-slate-700">
          <div className="flex items-center justify-between mb-2">
            <div>
              <h2 className="text-xl font-bold">Assign Resource</h2>
              <p className="text-sm text-slate-400 mt-1">{resourceTitle}</p>
            </div>
            <button
              onClick={onClose}
