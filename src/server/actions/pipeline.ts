...
export async function getPipeline() {
  ...
  try {
    // Only fetch Lead records - ProjectRequests are for client dashboard only
    const leads = await db.lead.findMany({
      include: {
        organization: true,
        project: true, // Include linked project to avoid showing converted leads
      },
      orderBy: {
        createdAt: "desc",
      },
...
/**
 * Get all clients (both Organizations and Users with CLIENT role)
 */
export async function getClients() {
  ...
  try {
    ...
    // Get organization emails to check for duplicates
    const orgEmails = new Set(organizations.map(org => org.email?.toLowerCase()).filter(Boolean));

    // Filter out client users who already have an organization (by matching email)
    const uniqueClientUsers = clientUsers.filter(user => 
      !orgEmails.has(user.email?.toLowerCase())
    );
...
/**
 * Get all admin users (for project assignment)
 */
