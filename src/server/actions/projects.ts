...
/**
 * Update project assignee (CEO only)
 */
export async function updateProjectAssignee(projectId: string, assigneeId: string | null) {
  ...
  try {
    ...
    // Get new assignee info if changing to someone
    ...
    // Emit feed event
    await emitFeedEvent(projectId, "assignee.changed", {
      from: project.assignee ? (project.assignee.name || project.assignee.email) : "Unassigned",
      to: newAssignee ? (newAssignee.name || newAssignee.email) : "Unassigned",
      by: session.user.name,
      timestamp: new Date().toISOString(),
    });
    ...
    console.error("[updateProjectAssignee] Error:", error);
