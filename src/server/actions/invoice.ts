 ...
 */
export async function sendInvoiceReceiptEmail(invoiceId: string) {  console.log(`[INVOICE RECEIPT] Starting sendInvoiceReceiptEmail for invoiceId: ${invoiceId}`);
  try {
    const invoice = await prisma.invoice.findUnique({
      where: { id: invoiceId },
      include: {
    ...
    if (invoice.status !== 'PAID') {      return { success: false, error: 'Invoice is not paid yet' };
    }

    const owner = invoice.organization.members[0]?.user;    if (!owner?.email) {      return { success: false, error: 'No customer email found' };
    }
