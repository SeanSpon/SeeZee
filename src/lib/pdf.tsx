...
const InvoicePDF = ({ invoice }: { invoice: InvoiceData }) => {
  ...
  const safeString = (value: any): string => {
    try {
      ...
      if (React.isValidElement(value)) {        // Try to salvage text from children if it's like <span>Text</span>
      ...
      return String(value);
    } catch (error) {      console.error('[PDF safeString] Unexpected error:', error);
      return '';
    }
  };
...
function deepSanitize(value: any, visited = new WeakSet()): any {
  ...
  if (React.isValidElement(value)) {    // Try to extract text from children if possible
    const children = (value as any).props?.children;
    if (children != null) {
...
export async function generateInvoicePDF(invoice: InvoiceData): Promise<Buffer> {
  try {    // Validate invoice data before rendering
    ...
    } catch (e) {      throw new Error('Invoice data cannot be serialized: ' + String(e));
    }
    ...
    // Note: deepCheckReact function not implemented, skipping verification
    // The deepSanitize above should handle React element removal
    ...
    const doc = <InvoicePDF invoice={ultraCleanInvoice} />;    const asPdf = pdf(doc);
