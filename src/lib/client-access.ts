...
export async function getClientAccessContext(
  ...
  const [memberships, leads, orgsByEmail] = await Promise.all([
    identity.userId
    ...
    identity.email
      ...
      : Promise.resolve([]),
    // CRITICAL: Also find organizations where email matches user's email
    // This allows admins to create projects for users by selecting them in the dropdown
    identity.email
  ...
  // Add organizations that match user's email
  orgsByEmail.forEach((org) => {
