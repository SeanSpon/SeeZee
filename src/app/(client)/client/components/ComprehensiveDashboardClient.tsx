...
export default function ComprehensiveDashboardClient({ 
  ...
  useEffect(() => {
    const fetchHoursBalance = async () => {      // If refresh=hours query param is present, always fetch fresh data
      ...
      if (shouldRefresh) {
        ...
        // Use the initial hours balance if no plan is selected and no refresh requested        setCurrentHoursBalance(hoursBalance);
      }
    };
