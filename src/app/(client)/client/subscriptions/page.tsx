...
export default async function SubscriptionsPage({ searchParams }: { searchParams: Promise<{ payment_required?: string; upgraded?: string; session_id?: string }> }) {
  ...
  // Get user's organization - check multiple ways
  // 1. Try organization membership
  ...
  // 2. If no membership, try finding organization by user's email
  if (!orgMembership && session.user.email) {
    const org = await db.organization.findFirst({
      where: { email: session.user.email },
    });

    if (org) {
      // Create a virtual membership object
      orgMembership = {
        id: 'virtual',
        userId: session.user.id,
