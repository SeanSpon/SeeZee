...
async function handleCheckoutSessionCompleted(session: Stripe.Checkout.Session) {
  try {
    ...
    if (session.metadata?.type === 'maintenance-subscription' || session.metadata?.type === 'maintenance-plan-subscription') {
      ...
      if (subscriptionId) {
        try {
          // Check if plan was already activated (e.g., by verify endpoint in dev mode)
          const existingPlan = await prisma.maintenancePlan.findUnique({
            where: { id: maintenancePlanId },
            select: { stripeSubscriptionId: true, status: true },
          });
          
          if (existingPlan?.stripeSubscriptionId === subscriptionId && existingPlan?.status === 'ACTIVE') {
            console.log(`[WEBHOOK] Maintenance plan ${maintenancePlanId} already activated with subscription ${subscriptionId}, skipping duplicate processing`);
            return;
          }
          ...
          // CRITICAL: Reset monthly usage counters for initial subscription
          ...
          console.log(`[WEBHOOK] ✅ Maintenance plan ${maintenancePlanId} activated: ${updateResult.supportHoursIncluded} hours available, period: ${periodStart.toISOString()} to ${periodEnd.toISOString()}`);
          ...
          console.error('[WEBHOOK] Failed to activate maintenance plan:', {
            maintenancePlanId,
            subscriptionId,
            error: error.message,
            stack: error.stack,
    ...
    if (stripeInvoice.subscription && stripeInvoice.billing_reason === 'subscription_cycle') {
      try {
        ...
        if (maintenancePlan) {
          ...
          console.log(`[WEBHOOK] ✅ Monthly hours reset for ${maintenancePlan.id}: ${maintenancePlan.supportHoursIncluded} hours available, ${newRolloverHours} rollover hours`);
          
          // Create rollover record for tracking
          if (unusedHours > 0) {
            await prisma.rolloverHours.create({
              data: {
                planId: maintenancePlan.id,
                hours: Math.min(unusedHours, maintenancePlan.rolloverCap - maintenancePlan.rolloverHours),
                hoursRemaining: Math.min(unusedHours, maintenancePlan.rolloverCap - maintenancePlan.rolloverHours),
                sourceMonth: maintenancePlan.currentPeriodStart || new Date(),
        ...
        console.error('[WEBHOOK] Failed to reset monthly hours on subscription renewal:', error.message);
        // Don't fail the webhook - invoice was still paid
      }
    }
...
async function handleSubscriptionCreated(subscription: Stripe.Subscription) {
  ...
  try {
    ...
    // Check if plan was already activated (e.g., by verify endpoint in dev mode)
    if (maintenancePlan.stripeSubscriptionId === subscription.id && maintenancePlan.status === 'ACTIVE') {
      console.log(`[WEBHOOK] Maintenance plan ${maintenancePlan.id} already activated with subscription ${subscription.id}, skipping duplicate processing`);
      return;
    }
    ...
    // CRITICAL: Reset monthly usage counters for new billing period
    const maintenancePlanUpdateData: {
      ...
      supportHoursUsed: 0, // Reset monthly hours used
      changeRequestsUsed: 0, // Reset change requests used
      urgentRequestsUsed: 0, // Reset urgent requests
      requestsToday: 0, // Reset daily counter
    };

    // Set billing period dates if available
    if (subscription.current_period_start && typeof subscription.current_period_start === 'number') {
      maintenancePlanUpdateData.currentPeriodStart = new Date(subscription.current_period_start * 1000);
    }
    ...
    console.log(`[WEBHOOK] ✅ Monthly hours allocated for ${maintenancePlan.id}: ${updateResult.supportHoursIncluded} hours available`);
