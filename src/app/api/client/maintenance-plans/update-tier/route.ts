...
export async function POST(req: NextRequest) {
  try {
    ...
    // CRITICAL: Only allow purchasing for ACTIVE projects (not just leads)
    let maintenancePlan;
    try {
      maintenancePlan = await prisma.maintenancePlan.findFirst({
        where: {
          project: {
            ...
            status: 'ACTIVE', // Only active projects can have maintenance plans
          },
        },
        include: {
          project: {
    ...
    // If no maintenance plan exists, check if user has an ACTIVE project
    if (!maintenancePlan) {
      // Get the first ACTIVE accessible project
      let project;
      try {
        project = await prisma.project.findFirst({
          where: {
            ...
            status: 'ACTIVE', // Only allow purchasing for active projects
          },
          include: {
            organization: true,
          },
      ...
      // If no active project exists, return helpful error
      if (!project) {
        console.log('[TIER UPDATE] No active project found for user');
        return NextResponse.json(
          { 
            error: "No active project found",
            message: "You need an active project before purchasing a maintenance plan. Please submit a project request or contact our team to get started.",
          },
          { status: 400 }
        );
      }
