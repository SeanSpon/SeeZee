...
export default function OnboardingProfilePage() {
  ...
  useEffect(() => {
    if (status === "authenticated" && session?.user?.id && !isRestored) {
      try {
        ...
        if (saved) {
          ...
          if (parsed.userId === session.user.id) {
            ...
            notification.className = 'fixed top-4 right-4 bg-emerald-500/90 text-white px-4 py-3 rounded-xl shadow-lg z-50 flex items-center gap-2 animate-slide-in';
  ...
  }, [status, session, isRestored]);

  // Auto-save draft as user types (debounced)
  useEffect(() => {
    if (!isRestored || status !== "authenticated" || !session?.user?.id) {
      return;
    }
  ...
  }, [formData, profileImage, session, status, isRestored]);
  ...
  if (status === "unauthenticated" || !session?.user?.id) {    if (typeof window !== "undefined") {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-8 shadow-2xl hover:border-white/20 transition-all duration-300">
          {/* Auto-save indicator */}
          {lastSaved && (
            <div className="mb-4 flex items-center gap-2 text-xs text-emerald-400/80">
