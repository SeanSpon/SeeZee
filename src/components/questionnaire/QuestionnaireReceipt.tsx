...
import { FileText, Download, Printer, User, Mail, Phone, Building2, Target, Calendar, MessageSquare, Clock } from 'lucide-react';

interface QuestionnaireReceiptProps {
  data: {
    package?: string; // Optional, not used anymore
    name: string;
    email: string;
    phone?: string;
    company?: string;
        ...
        <div className="mb-8">
          ...
          <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700 space-y-3">
            ...
            {data.estimatedBudget && (
              <div>
                <p className="text-white/60 text-sm">Estimated Budget</p>
                <p className="text-white font-medium">{data.estimatedBudget}</p>
              </div>
            )}
            {data.maintenanceInterest && (
              <div>
                <p className="text-white/60 text-sm">Maintenance Interest</p>
                <p className="text-white font-medium">{data.maintenanceInterest}</p>
              </div>
            )}
            {data.hourPackInterest && (
              <div>
                <p className="text-white/60 text-sm">Hour Pack Interest</p>
                <p className="text-white font-medium">{data.hourPackInterest}</p>
              </div>
            )}
